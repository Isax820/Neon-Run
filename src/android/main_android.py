...
def draw_jump_button(surface):
    cx, cy = JUMP_BUTTON_CENTER
    s = pygame.Surface((JUMP_BUTTON_RADIUS * 4, JUMP_BUTTON_RADIUS * 4), pygame.SRCALPHA)
    for i in range(6, 0, -1):
        alpha = 30 // i
        pygame.draw.circle(s, (*CYAN, alpha), (JUMP_BUTTON_RADIUS * 2, JUMP_BUTTON_RADIUS * 2),
                            JUMP_BUTTON_RADIUS + i * 3)
    surface.blit(s, (cx - JUMP_BUTTON_RADIUS * 2, cy - JUMP_BUTTON_RADIUS * 2))
    pygame.draw.circle(surface, (0, 40, 60), (cx, cy), JUMP_BUTTON_RADIUS)
    pygame.draw.circle(surface, CYAN, (cx, cy), JUMP_BUTTON_RADIUS, 3)
    label = "SAUT"
    lw, lh = font_small.size(label)
    draw_neon_text(surface, label, font_small, CYAN, cx - lw // 2, cy - lh // 2, glow=2)


def point_in_jump_button(x, y):
    cx, cy = JUMP_BUTTON_CENTER
    return (x - cx) ** 2 + (y - cy) ** 2 <= JUMP_BUTTON_RADIUS ** 2
...
class Player:
    ...
    def draw(self, surface):
        ...
        draw_neon_rect(surface, color, rect, width=2, glow=5)
    ...
    def draw(self, surface):
        ...
        if self.kind == "spike":
            ...
            for i in range(4, 0, -1):
                ...
                pygame.draw.polygon(s, (*self.color, alpha), shifted, 2 + i)
...
class Game:
    ...
    def spawn_platform(self):
        self.platforms.append(Platform(self.speed))
    ...
    def draw_ground(self, surface):
        ...
        for gx in range(-grid_spacing, WIDTH + grid_spacing, grid_spacing):
            ...
            s.fill((*CYAN, alpha))
            surface.blit(s, (x, GROUND_Y))

    def draw_hud(self, surface):
        score_text = f"SCORE {int(self.score):06d}"
        draw_neon_text(surface, score_text, font_med, CYAN, 20, 16, glow=3)

        hi_text = f"BEST {int(self.high_score):06d}"
        draw_neon_text(surface, hi_text, font_small, GRAY, 20, 56)

        spd_text = f"SPEED x{self.speed:.1f}"
        draw_neon_text(surface, spd_text, font_small, YELLOW, WIDTH - 220, 16)
    ...
    def screen_title(self, surface):
        ...
        draw_neon_text(surface, title, font_big, CYAN, WIDTH // 2 - tw // 2, 150, glow=8)

        sub = "PLATFORMER INFINI"
        sw = font_med.size(sub)[0]
        draw_neon_text(surface, sub, font_med, MAGENTA, WIDTH // 2 - sw // 2, 230, glow=4)

        inst = "TOUCHE l'ecran ou SPACE = SAUT (double saut possible)"
        iw = font_small.size(inst)[0]
        draw_neon_text(surface, inst, font_small, YELLOW, WIDTH // 2 - iw // 2, 330)

        inst2 = "Collecte les pieces - Evite les obstacles"
        i2w = font_small.size(inst2)[0]
        draw_neon_text(surface, inst2, font_small, WHITE, WIDTH // 2 - i2w // 2, 365)

        blink = int(self.frame / 20) % 2
        if blink:
            start = "TOUCHE l'ecran POUR COMMENCER"
            stw = font_med.size(start)[0]
            draw_neon_text(surface, start, font_med, GREEN, WIDTH // 2 - stw // 2, 450, glow=4)
    ...
    def screen_game_over(self, surface):
        ...
        go_text = "GAME OVER"
        gw = font_big.size(go_text)[0]
        draw_neon_text(surface, go_text, font_big, RED, WIDTH // 2 - gw // 2, 160, glow=8)

        sc_text = f"SCORE : {int(self.score):06d}"
        sw = font_med.size(sc_text)[0]
        draw_neon_text(surface, sc_text, font_med, YELLOW, WIDTH // 2 - sw // 2, 270, glow=4)

        hi_text = f"BEST : {int(self.high_score):06d}"
        hw = font_med.size(hi_text)[0]
        draw_neon_text(surface, hi_text, font_med, CYAN, WIDTH // 2 - hw // 2, 320, glow=3)

        blink = int(self.frame / 20) % 2
        if blink:
            retry = "TOUCHE l'ecran = REJOUER"
            rw = font_med.size(retry)[0]
            draw_neon_text(surface, retry, font_med, GREEN, WIDTH // 2 - rw // 2, 420, glow=4)
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if state == "title" and event.key == pygame.K_RETURN:
                        ...
                        if event.key in (pygame.K_SPACE, pygame.K_z, pygame.K_UP, pygame.K_w):
                            self.player.jump()
